...
public class CangureraDbContext : DbContext
{
	...
	{
		mb.Entity<ZonaCaliente>().Property((ZonaCaliente z) => z.Latitud).HasPrecision(9, 6);
		mb.Entity<ZonaCaliente>().Property((ZonaCaliente z) => z.Longitud).HasPrecision(9, 6);
		...
		mb.Entity<CatTipoDiscapacidad>().HasIndex((CatTipoDiscapacidad d) => d.Nombre).IsUnique();
		mb.Entity<Dispositivo>().Property((Dispositivo d) => d.MacAddress).UseCollation("SQL_Latin1_General_CP1_CI_AS");
		mb.Entity<Recorrido>().Property((Recorrido r) => r.Ruta_Coordenadas).HasColumnType("varchar(max)");
	}
}
